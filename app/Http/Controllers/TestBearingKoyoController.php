...
class TestBearingKoyoController extends Controller
{
    ...
    public function insert(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('Photo_Ng_Path')) {
                $photoPath1 = $request->file('Photo_Ng_Path')->store('bearing_koyo_photos', 'uploads');
            }
            if ($request->hasFile('Photo_Ng_Path_Two')) {
    ...
    {
        $date = Carbon::today();
        $records = Record::whereDate('Time_Record', $date)->where('Id_Comparison', 3)->with('comparison', 'tractor', 'part', 'user')->get();
